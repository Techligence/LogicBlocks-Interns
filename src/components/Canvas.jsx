...
const Canvas = () => {
  ...
  const handleSpriteClick = (index) => {
    ...
    if (selectedSprite !== null) {
      ...
      const currentWorkspaceDom = Blockly.Xml.workspaceToDom(
        Blockly.getMainWorkspace()
      );
      const currentWorkspaceXml = Blockly.Xml.domToText(currentWorkspaceDom);
      dispatch(
        setWorkspace({ index: selectedSprite, workspace: currentWorkspaceXml })
      );
    }
  ...
  useEffect(() => {
    ...
    storesprites[selectedSprite]?.position,
    storesprites[selectedSprite]?.angle,
  ]);
      ...
      >
        ...
        <div
          style={{
            ...
            background: selectedBackdrop
              ? `url(${selectedBackdrop}) center / cover no-repeat`
              : "none",
          }}
        />
        ...
        {storesprites.map((sprite, index) => (
          <Draggable
            key={index}
            bounds="parent"
            defaultPosition={sprite.position}
            position={sprite.position}
            onStart={() => {
              handleSpriteClick(index), console.log(sprite);
            }}
            onDrag={(e, data) => {
          ...
          >
            <Resizable
              ...
              style={{
                ...
                position: "absolute",
                boxShadow:
                  selectedSprite === index ? "0 0 0 2px #2196F3" : "none",
              }}
